...
export const flashcardsSlice = createSlice({
    ...
    reducers: {
        ...
        editFlashcard: (state, action) => {
            ...
            return {
                ...state, allFlashcards: updatedCards
            }
        },
        ...
        shuffleFlashcards: (state, action) => {
            let array = [...action.payload.flashcards];
            ...
            function shuffle() {
                ...
                while (currentIndex !== 0) {
                    ...
                    // Pick a remaining element (index 0 to currentIndex-1).
            ...
            return {
                ...state, allFlashcards: shuffle()
            }
        },
        resetFlashcardProgress: (state, action) => {
            let flashcards = action.payload.flashcards;

            return {
                ...state,
                allFlashcards: flashcards.map((card: FlashCard, i: number) => card.id === action.payload.id ? { ...card, knownCount: 0 } : card)
            }
        },
        updateFlashcardProgress: (state, action) => {
            let flashcards = action.payload.flashcards;

            return {
                ...state,
                allFlashcards: flashcards.map((card: FlashCard, i: number) => card.id === action.payload.id ? { ...card, knownCount: card.knownCount === 5 ? 5 : card.knownCount + 1 } : card)
            }
        }
    }
});

export const { addFlashcard, editFlashcard, deleteFlashcard, shuffleFlashcards, resetFlashcardProgress, updateFlashcardProgress } = flashcardsSlice.actions;
